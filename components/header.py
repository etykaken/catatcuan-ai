...
def render_sidebar(active_page: str = "beranda") -> None:
    ...
    with st.sidebar:
        st.markdown('<div class="sidebar-brand" aria-label="CatatCuan AI"><span class="brand-spark" aria-hidden="true">✦</span><span>CatatCuan</span><b>AI</b></div>', unsafe_allow_html=True)
        st.markdown('<div class="sidebar-tagline">Catat keuangan<br>semudah bercerita.</div>', unsafe_allow_html=True)
        st.markdown(f'''<nav class="app-nav" aria-label="Navigasi utama">
        ...
        </nav><div class="sidebar-spacer"></div><div class="sidebar-ai-card"><div class="ai-orb">✦</div><strong>CatatCuan AI</strong><p>Asisten keuangan pintar yang siap membantumu 24/7.</p><span>Pelajari lebih lanjut →</span></div>''', unsafe_allow_html=True)
        st.markdown(f'<div class="user-card"><div class="user-avatar">{html.escape(initials)}</div><div><strong>{html.escape(display_name)}</strong><small>{html.escape(subtitle)}</small></div></div>', unsafe_allow_html=True)
        if st.session_state.get("user") is None:
            ...
            if st.button(label, key="sidebar_auth_button", use_container_width=True):
                ...
                st.rerun()
        language_choice = st.selectbox("Bahasa / Language", ["ID", "EN"], index=0 if language == "id" else 1, key="sidebar_language")
...
def render_header() -> None:
    ...
    st.markdown(f'''<header class="dashboard-header"><div><h1>{html.escape(greeting)}</h1><p>{html.escape(helper)}</p></div><div class="header-actions"><div class="period-pill"><span aria-hidden="true">▣</span>&nbsp; {html.escape(period)} <span aria-hidden="true">⌄</span></div><div class="icon-pill" aria-label="Notifikasi"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg></div><div class="header-avatar">{html.escape(initials)}</div></div></header>''', unsafe_allow_html=True)
